...
api_url = "https://ethereum-mainnet.core.chainstack.com/92b8d8e613baea1750dc7435f17ceea3"  # YOU WILL NEED TO PROVIDE THE URL OF AN ETHEREUM NODE
provider = HTTPProvider(api_url)
web3 = Web3(provider)

def convert_ipfs_uri(ipfs_uri):
...
def get_ape_info(ape_id):
    ...
    ipfs_url = token_uri.replace("ipfs://", "https://gateway.pinata.cloud/ipfs/")

    response = requests.get(ipfs_url)
    metadata = response.json()

    data['image'] = metadata['image']
    eyes = next((attr['value'] for attr in metadata['attributes'] if attr['trait_type'] == 'Eyes'), None)
    data['eyes'] = eyes
